...
export default function DashboardLayout({ role, username, nav, onNavChange, onLogout, children }) {
  ...
  return (
    <div className="min-h-screen bg-gray-50 flex">
      <aside className={`${sidebarOpen ? 'w-56' : 'w-0 overflow-hidden'} bg-white border-r border-gray-100 flex-shrink-0 transition-all duration-200 relative`}>
          ...
          {navItems.map((item) => (
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 rounded-lg text-sm transition-colors ${
                nav === item.k ? 'bg-sky-50 text-sky-700 font-medium' : 'text-gray-600 hover:bg-gray-50'
              }`}
            >
              <item.i size={16} />
              {item.l}
        ...
        <div className="absolute bottom-0 w-56 p-3 border-t border-gray-100">
          <button onClick={onLogout} className="w-full flex items-center gap-2 px-3 py-2 rounded-lg text-sm text-gray-500 hover:bg-red-50 hover:text-red-600">
            <LogOut size={16} />
            Sign out
          </button>
        </div>
      ...
      <main className="flex-1 flex flex-col min-h-screen">
        <header className="bg-white border-b border-gray-100 px-5 py-3 flex items-center justify-between">
          <div className="flex items-center gap-3">
            <button onClick={() => setSidebarOpen((v) => !v)} className="text-gray-400 hover:text-gray-600">
              <Menu size={20} />
            </button>
